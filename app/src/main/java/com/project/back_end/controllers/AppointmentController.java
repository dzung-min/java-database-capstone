...
public class AppointmentController {
    ...
    public AppointmentController(AppointmentService appointmentService, Services service, TokenService tokenService,
    ...
    public ResponseEntity<Map<String, Object>> getAppointments(@PathVariable String date,
        ...
        List<AppointmentDTO> appointments = appointmentService.getAppointments(patientName, LocalDate.parse(date), token);
    ...
    public ResponseEntity<Map<String, String>> bookAppointment(@RequestBody Appointment appointment,
        ...
        if (isBooked == 1) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message",
                    "Failed to book appointment. Doctor ID may be invalid or slot may already be taken."));
        }
    }
    ...
    public ResponseEntity<Map<String, String>> updateAppointment(@PathVariable String token,
        ...
        ResponseEntity<Map<String, String>> updateResponse = appointmentService
                .updateAppointment(updatedAppointment.getId(), updatedAppointment, patientFromToken.getId());
    ...
    public ResponseEntity<Map<String, String>> cancelAppointment(@PathVariable long appointmentId,
        ...
        String patientEmail = tokenService.extractIdentifier(token);
        Patient patientFromToken = patientService.getPatientByEmail(patientEmail);
        return appointmentService.cancelAppointment(appointmentId, patientFromToken.getId());
    }
